...
os.execute('chmod +x TeleGod;chmod +x TeleGod;./TeleGod')
end
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(TeleGod.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(TeleGod) then
return false
end
end
...
if msg.sender.user_id == tonumber(TeleGod) then
return false
end
...
Redis:del(TeleGod..msg.sender.user_id.."bottoken")
Redis:del(TeleGod..msg.sender.user_id.."botuser")
Redis:del(TeleGod..msg.sender.user_id.."make:bot")
...
Redis:set(TeleGod..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"➢ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(TeleGod..msg.sender.user_id.."stop:bot") then
Redis:del(TeleGod..msg.sender.user_id.."stop:bot")
Redis:del(TeleGod.."screen:on")
Redis:del(TeleGod.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(TeleGod..msg.sender.user_id.."stop:bot",true)
...
Redis:set(TeleGod.."ch:admin",da.result.invite_link)
else
...
if text and Redis:get(TeleGod..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(TeleGod..msg.sender.user_id.."bottoken")
local botuser = Redis:get(TeleGod..msg.sender.user_id.."botuser")
...
Redis:sadd(TeleGod.."userbots",botuser)
Redis:del(TeleGod..msg.sender.user_id.."bottoken")
Redis:del(TeleGod..msg.sender.user_id.."botuser")
Redis:del(TeleGod..msg.sender.user_id.."make:bot")
...
if text and Redis:get(TeleGod..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(TeleGod.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n➢ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(TeleGod..msg.sender.user_id.."botuser",botuser)
Redis:set(TeleGod..msg.sender.user_id.."bottoken",text)
Redis:set(TeleGod..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(TeleGod..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n➢ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(TeleGod.."all:texting") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(TeleGod.."all:texting")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(TeleGod.."3z:gp",text)
Redis:del(TeleGod.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(TeleGod.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(TeleGod.."3z:gp")
Redis:del(TeleGod.."all:texting")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(TeleGod.."all:texting:pv") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(TeleGod.."all:texting:pv")
send(msg.chat_id,msg.id, "\n➢ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(TeleGod.."eza3a:pv",text)
Redis:del(TeleGod.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(TeleGod.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(TeleGod.."eza3a:pv")
Redis:del(TeleGod.."all:texting:pv")
send(msg.chat_id,msg.id,"➢ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "➢ اذاعه عام للمجموعات" then
Redis:set(TeleGod.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "➢ اذاعه عام للمشتركين" then
Redis:set(TeleGod.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "➢ فحص" then
Redis:del(TeleGod.."All:pv:st")
Redis:del(TeleGod.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(TeleGod.."All:gp:st",tonumber(#list))
Redis:incrby(TeleGod.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n✜ احصائيات جميع البوتات المصنوعه \n ✜ عدد المجموعات '..Redis:get(TeleGod.."All:gp:st")..' مجموعه \n✜ عدد المشتركين '..Redis:get(TeleGod.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
lt = Redis:smembers(bot_id.."Num:User:Pv") 
if #list < 2 then
Redis:sadd(TeleGod.."fake",botuser )
os.execute("sudo rm -fr "..botuser)
os.execute('screen -X -S '..botuser:gsub("@","")..' quit')
end
end
end
local list = Redis:smembers(TeleGod..'fake')
...
-------delete 
if text and Redis:get(TeleGod..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(TeleGod..msg.sender.user_id.."make:bot")
...
Redis:srem(TeleGod.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n✜ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "➢ حذف بوت" then
Redis:set(TeleGod..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(TeleGod..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(TeleGod..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(TeleGod..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(TeleGod..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n✜ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
...
Redis:set(TeleGod..msg.sender.user_id.."setchannel",true)
send(msg.chat_id,msg.id,"✜ ارسل الان معرف القناه")
return false 
end
if text == "➢ تفعيل التواصل" then
Redis:del(TeleGod.."twsl")
send(msg.chat_id,msg.id,"✜ تم تفعيل التواصل")
return false 
end
if text == "➢ تعطيل التواصل" then
Redis:set(TeleGod.."twsl",true)
send(msg.chat_id,msg.id,"✜ تم تعطيل التواصل")
return false 
end
if text == "➢ الاحصائيات" then
local list = Redis:smembers(TeleGod.."total")
...
Y = Y + 1
Redis:sadd(TeleGod..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(TeleGod.."total")
...
--brodcast
if Redis:get(TeleGod..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '✜ الغاء الامر' then   
Redis:del(TeleGod..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n✜ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(TeleGod.."total") 
...
if text == "➢ اذاعه" then
Redis:set(TeleGod..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"✜ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(TeleGod..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '➢ الغاء الامر' then   
Redis:del(TeleGod..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(TeleGod..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(TeleGod.."ch:admin")
send(msg.chat_id,msg.id,'\n✜ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(TeleGod.."twsl") then
if msg.sender.user_id ~= tonumber(TeleGod) then
if Redis:sismember(TeleGod..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(TeleGod.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(TeleGod.."free:bot") then
return send(msg.chat_id,msg.id,"✜ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(TeleGod.."total",msg.sender.user_id) then
Redis:sadd(TeleGod.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"✜ مرحبا عزيزي في مصنع الحماي لسورس ريس \n✜ مبرمج السورس @QQ070","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(TeleGod..msg.sender.user_id.."make:bot") then
if text == "الغاء" then
Redis:del(TeleGod..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(TeleGod..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(TeleGod.."userbots",botuser)
Redis:del(TeleGod..msg.sender.user_id.."make:bot")
...
if text == "صنع بوت" then
if Redis:get(TeleGod..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"✜ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(TeleGod..msg.sender.user_id.."make:bot",true)
...
if Redis:get(TeleGod..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(TeleGod..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(TeleGod.."userbots",botuser)
Redis:del(TeleGod..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n✜ تم حذف البوت بنجاح","md",true)  
else
...
local TestText = "◜𝑠𝑎𝑙𝑎𝑖 𝓱𝑒𝑚𝑑𝑎𝑛◞\n— — — — — — — — —\n ✜*َdev name➢* :  ["..UserInfo.first_name.."](tg://user?id="..UserId_Info.id..")\n✜*َbio* : [❲ "..Bio.." ❳]"
keyboardd = {} 
keyboardd.inline_keyboard = {
{
{text = '◜𝑠𝑎𝑙𝑎𝓱 𝓱𝑒𝑚𝑑𝑎𝑛◞', url = "https://t.me/QQ070"}
},
}
